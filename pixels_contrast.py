...
def apply_brightness_contrast(img, contrast):
    # Apply brightness and contrast
    img = np.int16(img) * (contrast/127 + 1) - contrast #+ brightness
...
def on_trackbar_change(value):
    ...
    contrast = cv2.getTrackbarPos('Contrast', 'Pixelizer')
    pixel_size = pixel_size if pixel_size > 3 else 3
    br_contr_corrected  = apply_brightness_contrast(img, contrast)
    pixelized = pixeller(br_contr_corrected, pixel_size=pixel_size)
...
if __name__ == '__main__':
    ...
    cv2.createTrackbar('Contrast', 'Pixelizer',
                    contrast, 100, on_trackbar_change)
